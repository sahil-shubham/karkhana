defmodule Karkhana.LiveE2ETest do
  ...
  @live_e2e_skip_reason if(System.get_env("KARKHANA_RUN_LIVE_E2E") != "1",
                          do: "set KARKHANA_RUN_LIVE_E2E=1 to enable the real Linear/Codex end-to-end test"
                        )

  @team_query """
  query KarkhanaLiveE2ETeam($key: String!) {
  ...
  mutation KarkhanaLiveE2ECreateProject($name: String!, $teamIds: [String!]!) {
  ...
  mutation KarkhanaLiveE2ECreateIssue(
    $teamId: String!
    $projectId: String!
    $title: String!
    $description: String!
  ...
  query KarkhanaLiveE2EProjectStatuses {
  ...
  @issue_details_query """
  query KarkhanaLiveE2EIssueDetails($id: String!) {
    issue(id: $id) {
      id
      identifier
      state {
  ...
  mutation KarkhanaLiveE2ECompleteProject($id: String!, $statusId: String!, $completedAt: DateTime!) {
  ...
  defp live_prompt(project_slug) do
    """
    You are running a real Karkhana end-to-end test.
  ...
  defp run_live_issue_flow!(backend) when backend in [:local, :ssh] do
    run_id = "karkhana-live-e2e-#{backend}-#{System.unique_integer([:positive])}"
    ...
    team_key = System.get_env("KARKHANA_LIVE_LINEAR_TEAM_KEY") || @default_team_key
    ...
    try do
      ...
      project =
        create_project!(
          team["id"],
          "Karkhana Live E2E #{backend} #{System.unique_integer([:positive])}"
        )
      ...
      issue =
        create_issue!(
          ...
          project["id"],
          active_state["id"],
          "Karkhana live e2e #{backend} issue for #{project["name"]}"
        )
  ...
  defp live_docker_worker_setup!(run_id, test_root) when is_binary(run_id) and is_binary(test_root) do
    ...
    previous_ssh_config = System.get_env("KARKHANA_SSH_CONFIG")

    base_cleanup = fn ->
      restore_env("KARKHANA_SSH_CONFIG", previous_ssh_config)
    ...
    result =
      try do
        ...
        System.put_env("KARKHANA_SSH_CONFIG", config_path)
  ...
  defp live_ssh_worker_hosts do
    System.get_env("KARKHANA_LIVE_SSH_WORKER_HOSTS", "")
  ...
  defp docker_compose_env(worker_ports, auth_json_path, authorized_key_path)
    ...
    [
      {"KARKHANA_LIVE_DOCKER_AUTH_JSON", auth_json_path},
      {"KARKHANA_LIVE_DOCKER_AUTHORIZED_KEY", authorized_key_path},
      {"KARKHANA_LIVE_DOCKER_WORKER_1_PORT", Integer.to_string(Enum.at(worker_ports, 0))},
      {"KARKHANA_LIVE_DOCKER_WORKER_2_PORT", Integer.to_string(Enum.at(worker_ports, 1))}
    ]
  end
