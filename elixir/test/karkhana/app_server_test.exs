defmodule Karkhana.AppServerTest do
  ...
  test "app server rejects the workspace root and paths outside workspace root" do
    test_root =
      Path.join(
        System.tmp_dir!(),
        "karkhana-app-server-cwd-guard-#{System.unique_integer([:positive])}"
      )
  ...
  test "app server rejects symlink escape cwd paths under the workspace root" do
    test_root =
      Path.join(
        System.tmp_dir!(),
        "karkhana-app-server-symlink-cwd-guard-#{System.unique_integer([:positive])}"
      )
  ...
  test "app server passes explicit turn sandbox policies through unchanged" do
    test_root =
      Path.join(
        System.tmp_dir!(),
        "karkhana-app-server-supported-turn-policies-#{System.unique_integer([:positive])}"
      )
    ...
    try do
      ...
      #!/bin/sh
      trace_file="${KARKHANA_TEST_CODEx_TRACE:-/tmp/codex-supported-turn-policies.trace}"
      count=0
  ...
  test "app server marks request-for-input events as a hard failure" do
    test_root =
      Path.join(
        System.tmp_dir!(),
        "karkhana-app-server-input-#{System.unique_integer([:positive])}"
      )
    ...
    try do
      ...
      File.write!(codex_binary, """
      #!/bin/sh
      trace_file="${KARKHANA_TEST_CODEx_TRACE:-/tmp/codex-input.trace}"
      count=0
      while IFS= read -r line; do
        count=$((count + 1))
        printf 'JSON:%s\\n' \"$line\" >> \"$trace_file\"
  ...
  test "app server fails when command execution approval is required under safer defaults" do
    test_root =
      Path.join(
        System.tmp_dir!(),
        "karkhana-app-server-approval-required-#{System.unique_integer([:positive])}"
      )
  ...
  test "app server auto-approves command execution approval requests when approval policy is never" do
    test_root =
      Path.join(
        System.tmp_dir!(),
        "karkhana-app-server-auto-approve-#{System.unique_integer([:positive])}"
      )
    ...
    try do
      ...
      File.write!(codex_binary, """
      #!/bin/sh
      trace_file="${KARKHANA_TEST_CODex_TRACE:-/tmp/codex-auto-approve.trace}"
      count=0
      while IFS= read -r line; do
        count=$((count + 1))
        printf 'JSON:%s\\n' \"$line\" >> \"$trace_file\"
  ...
  test "app server auto-approves MCP tool approval prompts when approval policy is never" do
    test_root =
      Path.join(
        System.tmp_dir!(),
        "karkhana-app-server-tool-user-input-auto-approve-#{System.unique_integer([:positive])}"
      )
    ...
    try do
      ...
      File.write!(codex_binary, """
      #!/bin/sh
      trace_file="${KARKHANA_TEST_CODEx_TRACE:-/tmp/codex-tool-user-input-auto-approve.trace}"
      count=0
      while IFS= read -r line; do
        count=$((count + 1))
        printf 'JSON:%s\\n' \"$line\" >> \"$trace_file\"
  ...
  test "app server sends a generic non-interactive answer for freeform tool input prompts" do
    test_root =
      Path.join(
        System.tmp_dir!(),
        "karkhana-app-server-tool-user-input-required-#{System.unique_integer([:positive])}"
      )
  ...
  test "app server sends a generic non-interactive answer for option-based tool input prompts" do
    test_root =
      Path.join(
        System.tmp_dir!(),
        "karkhana-app-server-tool-user-input-options-#{System.unique_integer([:positive])}"
      )
    ...
    try do
      ...
      File.write!(codex_binary, """
      #!/bin/sh
      trace_file="${KARKHANA_TEST_CODEx_TRACE:-/tmp/codex-tool-user-input-options.trace}"
      count=0
      while IFS= read -r line; do
        count=$((count + 1))
        printf 'JSON:%s\\n' \"$line\" >> \"$trace_file\"
  ...
  test "app server rejects unsupported dynamic tool calls without stalling" do
    test_root =
      Path.join(
        System.tmp_dir!(),
        "karkhana-app-server-tool-call-#{System.unique_integer([:positive])}"
      )
    ...
    try do
      ...
      File.write!(codex_binary, """
      #!/bin/sh
      trace_file="${KARKHANA_TEST_CODEx_TRACE:-/tmp/codex-tool-call.trace}"
      count=0
      while IFS= read -r line; do
        count=$((count + 1))
        printf 'JSON:%s\\n' \"$line\" >> \"$trace_file\"
  ...
  test "app server executes supported dynamic tool calls and returns the tool result" do
    test_root =
      Path.join(
        System.tmp_dir!(),
        "karkhana-app-server-supported-tool-call-#{System.unique_integer([:positive])}"
      )
    ...
    try do
      ...
      File.write!(codex_binary, """
      #!/bin/sh
      trace_file="${KARKHANA_TEST_CODEx_TRACE:-/tmp/codex-supported-tool-call.trace}"
      count=0
      while IFS= read -r line; do
        count=$((count + 1))
        printf 'JSON:%s\\n' \"$line\" >> \"$trace_file\"
  ...
  test "app server emits tool_call_failed for supported tool failures" do
    test_root =
      Path.join(
        System.tmp_dir!(),
        "karkhana-app-server-tool-call-failed-#{System.unique_integer([:positive])}"
      )
    ...
    try do
      ...
      File.write!(codex_binary, """
      #!/bin/sh
      trace_file="${KARKHANA_TEST_CODEx_TRACE:-/tmp/codex-tool-call-failed.trace}"
      count=0
      while IFS= read -r line; do
        count=$((count + 1))
        printf 'JSON:%s\\n' \"$line\" >> \"$trace_file\"
  ...
  test "app server buffers partial JSON lines until newline terminator" do
    test_root =
      Path.join(
        System.tmp_dir!(),
        "karkhana-app-server-partial-line-#{System.unique_integer([:positive])}"
      )
  ...
  test "app server captures codex side output and logs it through Logger" do
    test_root =
      Path.join(
        System.tmp_dir!(),
        "karkhana-app-server-stderr-#{System.unique_integer([:positive])}"
      )
  ...
  test "app server emits malformed events for JSON-like protocol lines that fail to decode" do
    test_root =
      Path.join(
        System.tmp_dir!(),
        "karkhana-app-server-malformed-protocol-#{System.unique_integer([:positive])}"
      )
  ...
  test "app server launches over ssh for remote workers" do
    test_root =
      Path.join(
        System.tmp_dir!(),
        "karkhana-app-server-remote-ssh-#{System.unique_integer([:positive])}"
      )
