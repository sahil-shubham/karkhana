defmodule Karkhana.OrchestratorStatusTest do
  ...
  test "status dashboard renders dashboard url on its own line when server port is configured" do
    previous_port_override = Application.get_env(:karkhana, :server_port_override)

    on_exit(fn ->
      if is_nil(previous_port_override) do
        Application.delete_env(:karkhana, :server_port_override)
      else
        Application.put_env(:karkhana, :server_port_override, previous_port_override)
      end
    end)

    Application.put_env(:karkhana, :server_port_override, 4000)
  ...
  test "application configures a rotating file logger handler" do
    assert {:ok, handler_config} = :logger.get_handler_config(:karkhana_disk_log)
  ...
  test "application stop renders offline status" do
    rendered =
      ExUnit.CaptureIO.capture_io(fn ->
        assert :ok = Karkhana.Application.stop(:normal)
      end)
