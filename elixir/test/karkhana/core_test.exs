defmodule Karkhana.CoreTest do
  ...
  test "current WORKFLOW.md file is valid and complete" do
    ...
    assert is_map(hooks)
    assert Map.get(hooks, "after_create") =~ "git clone --depth 1 https://github.com/sahil-shubham/karkhana ."
  ...
  test "Karkhana.start_link delegates to the orchestrator" do
    write_workflow_file!(Workflow.workflow_file_path(), tracker_kind: "memory")
    Application.put_env(:karkhana, :memory_tracker_issues, [])
    orchestrator_pid = Process.whereis(Karkhana.Orchestrator)

    on_exit(fn ->
      if is_nil(Process.whereis(Karkhana.Orchestrator)) do
        case Supervisor.restart_child(Karkhana.Supervisor, Karkhana.Orchestrator) do
          {:ok, _pid} -> :ok
    ...
    if is_pid(orchestrator_pid) do
      assert :ok = Supervisor.terminate_child(Karkhana.Supervisor, Karkhana.Orchestrator)
    end

    assert {:ok, pid} = Karkhana.start_link()
    assert Process.whereis(Karkhana.Orchestrator) == pid
  ...
  test "non-active issue state stops running agent without cleaning workspace" do
    test_root =
      Path.join(
        System.tmp_dir!(),
        "karkhana-nonactive-reconcile-#{System.unique_integer([:positive])}"
      )
  ...
  test "terminal issue state stops running agent and cleans workspace" do
    test_root =
      Path.join(
        System.tmp_dir!(),
        "karkhana-terminal-reconcile-#{System.unique_integer([:positive])}"
      )
  ...
  test "missing running issues stop active agents without cleaning the workspace" do
    test_root =
      Path.join(
        System.tmp_dir!(),
        "karkhana-missing-running-reconcile-#{System.unique_integer([:positive])}"
      )

    previous_memory_issues = Application.get_env(:karkhana, :memory_tracker_issues)
    ...
    try do
      ...
      assert {:ok, workspace} =
               Karkhana.PathSafety.canonicalize(Path.join(test_root, issue_identifier))
  ...
  defp restore_app_env(key, nil), do: Application.delete_env(:karkhana, key)
  defp restore_app_env(key, value), do: Application.put_env(:karkhana, key, value)
  ...
  test "prompt builder reports workflow load failures separately from template parse errors" do
    original_workflow_path = Workflow.workflow_file_path()
    workflow_store_pid = Process.whereis(Karkhana.WorkflowStore)

    on_exit(fn ->
      Workflow.set_workflow_file_path(original_workflow_path)

      if is_pid(workflow_store_pid) and is_nil(Process.whereis(Karkhana.WorkflowStore)) do
        Supervisor.restart_child(Karkhana.Supervisor, Karkhana.WorkflowStore)
      end
    end)

    assert :ok = Supervisor.terminate_child(Karkhana.Supervisor, Karkhana.WorkflowStore)
  ...
  test "agent runner keeps workspace after successful codex run" do
    test_root =
      Path.join(
        System.tmp_dir!(),
        "karkhana-agent-runner-retain-workspace-#{System.unique_integer([:positive])}"
      )
  ...
  test "agent runner forwards timestamped codex updates to recipient" do
    test_root =
      Path.join(
        System.tmp_dir!(),
        "karkhana-agent-runner-updates-#{System.unique_integer([:positive])}"
      )
  ...
  test "agent runner surfaces ssh startup failures instead of silently hopping hosts" do
    test_root =
      Path.join(
        System.tmp_dir!(),
        "karkhana-agent-runner-single-host-#{System.unique_integer([:positive])}"
      )
    ...
    try do
      ...
      case "$*" in
        *worker-a*"__KARKHANA_WORKSPACE__"*)
          printf '%s\\n' 'worker-a prepare failed' >&2
          exit 75
          ;;
        *worker-b*"__KARKHANA_WORKSPACE__"*)
          printf '%s\\t%s\\t%s\\n' '__KARKHANA_WORKSPACE__' '1' '/remote/home/.karkhana-remote-workspaces/MT-SSH-FAILOVER'
          exit 0
          ;;
        *)
          exit 0
  ...
  test "agent runner continues with a follow-up turn while the issue remains active" do
    test_root =
      Path.join(
        System.tmp_dir!(),
        "karkhana-agent-runner-continuation-#{System.unique_integer([:positive])}"
      )
    ...
    try do
      ...
      File.write!(codex_binary, """
      #!/bin/sh
      trace_file="${KARKHANA_TEST_CODEx_TRACE:-/tmp/codex.trace}"
      run_id="$(date +%s%N)-$$"
      printf 'RUN:%s\\n' "$run_id" >> "$trace_file"
      count=0
      ...
      File.chmod!(codex_binary, 0o755)
      System.put_env("KARKHANA_TEST_CODEx_TRACE", trace_file)

      on_exit(fn -> System.delete_env("KARKHANA_TEST_CODEx_TRACE") end)
    ...
    after
      System.delete_env("KARKHANA_TEST_CODEx_TRACE")
      File.rm_rf(test_root)
    end
  end
  ...
  test "agent runner stops continuing once agent.max_turns is reached" do
    test_root =
      Path.join(
        System.tmp_dir!(),
        "karkhana-agent-runner-max-turns-#{System.unique_integer([:positive])}"
      )
  ...
  test "app server starts with workspace cwd and expected startup command" do
    test_root =
      Path.join(
        System.tmp_dir!(),
        "karkhana-app-server-args-#{System.unique_integer([:positive])}"
      )
    ...
    try do
      ...
      previous_trace = System.get_env("KARKHANA_TEST_CODex_TRACE")

      on_exit(fn ->
        if is_binary(previous_trace) do
          System.put_env("KARKHANA_TEST_CODex_TRACE", previous_trace)
        else
          System.delete_env("KARKHANA_TEST_CODex_TRACE")
        end
      end)
      ...
      File.write!(codex_binary, """
      #!/bin/sh
      trace_file="${KARKHANA_TEST_CODex_TRACE:-/tmp/codex-args.trace}"
      count=0
      printf 'ARGV:%s\\n' \"$*\" >> \"$trace_file\"
      printf 'CWD:%s\\n' \"$PWD\" >> \"$trace_file\"
  ...
  test "app server startup command supports codex args override from workflow config" do
    test_root =
      Path.join(
        System.tmp_dir!(),
        "karkhana-app-server-custom-args-#{System.unique_integer([:positive])}"
      )
    ...
    try do
      ...
      File.write!(codex_binary, """
      #!/bin/sh
      trace_file="${KARKHANA_TEST_CODex_TRACE:-/tmp/codex-custom-args.trace}"
      count=0
      printf 'ARGV:%s\\n' \"$*\" >> \"$trace_file\"
  ...
  test "app server startup payload uses configurable approval and sandbox settings from workflow config" do
    test_root =
      Path.join(
        System.tmp_dir!(),
        "karkhana-app-server-policy-overrides-#{System.unique_integer([:positive])}"
      )
    ...
    try do
      ...
      File.write!(codex_binary, """
      #!/bin/sh
      trace_file="${KARKHANA_TEST_CODex_TRACE:-/tmp/codex-policy-overrides.trace}"
      count=0
