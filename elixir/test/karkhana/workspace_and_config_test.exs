defmodule Karkhana.WorkspaceAndConfigTest do
  use Karkhana.TestSupport
  alias Ecto.Changeset
  alias Karkhana.Config.Schema
  alias Karkhana.Config.Schema.{Codex, StringOrMap}
  ...
  test "workspace bootstrap can be implemented in after_create hook" do
    test_root =
      Path.join(
        System.tmp_dir!(),
        "karkhana-workspace-hook-bootstrap-#{System.unique_integer([:positive])}"
      )
  ...
  test "workspace path is deterministic per issue identifier" do
    workspace_root =
      Path.join(
        System.tmp_dir!(),
        "karkhana-workspace-deterministic-#{System.unique_integer([:positive])}"
      )
  ...
  test "workspace reuses existing issue directory without deleting local changes" do
    workspace_root =
      Path.join(
        System.tmp_dir!(),
        "karkhana-workspace-reuse-#{System.unique_integer([:positive])}"
      )
  ...
  test "workspace replaces stale non-directory paths" do
    workspace_root =
      Path.join(
        System.tmp_dir!(),
        "karkhana-workspace-stale-path-#{System.unique_integer([:positive])}"
      )
    ...
    try do
      ...
      assert {:ok, canonical_workspace} = Karkhana.PathSafety.canonicalize(stale_workspace)
  ...
  test "workspace rejects symlink escapes under the configured root" do
    test_root =
      Path.join(
        System.tmp_dir!(),
        "karkhana-workspace-symlink-#{System.unique_integer([:positive])}"
      )
    ...
    try do
      ...
      assert {:ok, canonical_outside_root} = Karkhana.PathSafety.canonicalize(outside_root)
      assert {:ok, canonical_workspace_root} = Karkhana.PathSafety.canonicalize(workspace_root)
  ...
  test "workspace canonicalizes symlinked workspace roots before creating issue directories" do
    test_root =
      Path.join(
        System.tmp_dir!(),
        "karkhana-workspace-root-symlink-#{System.unique_integer([:positive])}"
      )
    ...
    try do
      ...
      assert {:ok, canonical_workspace} =
               Karkhana.PathSafety.canonicalize(Path.join(actual_root, "MT-LINK"))
  ...
  test "workspace remove rejects the workspace root itself with a distinct error" do
    workspace_root =
      Path.join(
        System.tmp_dir!(),
        "karkhana-workspace-root-remove-#{System.unique_integer([:positive])}"
      )
    ...
    try do
      ...
      assert {:ok, canonical_workspace_root} =
               Karkhana.PathSafety.canonicalize(workspace_root)
  ...
  test "workspace surfaces after_create hook failures" do
    workspace_root =
      Path.join(
        System.tmp_dir!(),
        "karkhana-workspace-hook-failure-#{System.unique_integer([:positive])}"
      )
  ...
  test "workspace surfaces after_create hook timeouts" do
    workspace_root =
      Path.join(
        System.tmp_dir!(),
        "karkhana-workspace-hook-timeout-#{System.unique_integer([:positive])}"
      )
  ...
  test "workspace creates an empty directory when no bootstrap hook is configured" do
    workspace_root =
      Path.join(
        System.tmp_dir!(),
        "karkhana-workspace-empty-#{System.unique_integer([:positive])}"
      )
    ...
    try do
      ...
      assert {:ok, canonical_workspace} = Karkhana.PathSafety.canonicalize(workspace)
  ...
  test "workspace removes all workspaces for a closed issue identifier" do
    workspace_root =
      Path.join(
        System.tmp_dir!(),
        "karkhana-issue-workspace-cleanup-#{System.unique_integer([:positive])}"
      )
  ...
  test "workspace cleanup handles missing workspace root" do
    missing_root =
      Path.join(
        System.tmp_dir!(),
        "karkhana-missing-workspaces-#{System.unique_integer([:positive])}"
      )
  ...
  test "linear client paginates issue state fetches by id beyond one page" do
    ...
    assert query =~ "KarkhanaLinearIssuesById"
  ...
  test "workspace remove returns error information for missing directory" do
    random_path =
      Path.join(
        System.tmp_dir!(),
        "karkhana-missing-#{System.unique_integer([:positive])}"
      )
  ...
  test "workspace hooks support multiline YAML scripts and run at lifecycle boundaries" do
    test_root =
      Path.join(
        System.tmp_dir!(),
        "karkhana-workspace-hooks-#{System.unique_integer([:positive])}"
      )
  ...
  test "workspace remove continues when before_remove hook fails" do
    test_root =
      Path.join(
        System.tmp_dir!(),
        "karkhana-workspace-hooks-fail-#{System.unique_integer([:positive])}"
      )
  ...
  test "workspace remove continues when before_remove hook fails with large output" do
    test_root =
      Path.join(
        System.tmp_dir!(),
        "karkhana-workspace-hooks-large-fail-#{System.unique_integer([:positive])}"
      )
  ...
  test "workspace remove continues when before_remove hook times out" do
    previous_timeout = Application.get_env(:karkhana, :workspace_hook_timeout_ms)

    on_exit(fn ->
      if is_nil(previous_timeout) do
        Application.delete_env(:karkhana, :workspace_hook_timeout_ms)
      else
        Application.put_env(:karkhana, :workspace_hook_timeout_ms, previous_timeout)
      end
    end)

    Application.put_env(:karkhana, :workspace_hook_timeout_ms, 10)

    test_root =
      Path.join(
        System.tmp_dir!(),
        "karkhana-workspace-hooks-timeout-#{System.unique_integer([:positive])}"
      )
  ...
  test "config reads defaults for optional settings" do
    ...
    assert config.workspace.root == Path.join(System.tmp_dir!(), "karkhana_workspaces")
    ...
    assert {:ok, canonical_default_workspace_root} =
             Karkhana.PathSafety.canonicalize(Path.join(System.tmp_dir!(), "karkhana_workspaces"))
    ...
    explicit_root =
      Path.join(
        System.tmp_dir!(),
        "karkhana-explicit-sandbox-root-#{System.unique_integer([:positive])}"
      )
    ...
    assert Config.agent_turn_sandbox_policy(explicit_workspace) == %{
             "type" => "workspaceWrite",
             "writableRoots" => [explicit_workspace, explicit_cache]
           }
  ...
  test "config resolves $VAR references for env-backed secret and path values" do
    ...
    workspace_root = Path.join("/tmp", "karkhana-workspace-root")
  ...
  test "schema parse normalizes policy keys and env-backed fallbacks" do
    ...
    assert settings.workspace.root == Path.join(System.tmp_dir!(), "karkhana_workspaces")
  ...
  test "schema resolves sandbox policies from explicit and default workspaces" do
    ...
    assert Schema.resolve_turn_sandbox_policy(%Schema{
             ...
             "type" => "workspaceWrite",
             "writableRoots" => [Path.expand(Path.join(System.tmp_dir!(), "karkhana_workspaces"))],
             "readOnlyAccess" => %{"type" => "fullAccess"},
             "networkAccess" => false,
             "excludeTmpdirEnvVar" => false,
             "excludeSlashTmp" => false
  ...
  test "schema keeps workspace roots raw while sandbox helpers expand only for local use" do
    assert {:ok, settings} =
             Schema.parse(%{
               workspace: %{root: "~/.karkhana-workspaces"},
               codex: %{}
             })

    assert settings.workspace.root == "~/.karkhana-workspaces"

    assert Schema.resolve_turn_sandbox_policy(settings) == %{
             "type" => "workspaceWrite",
             "writableRoots" => [Path.expand("~/.karkhana-workspaces")],
             "readOnlyAccess" => %{"type" => "fullAccess"},
             "networkAccess" => false,
             "excludeTmpdirEnvVar" => false,
             "excludeSlashTmp" => false
    ...
    assert remote_policy == %{
             "type" => "workspaceWrite",
             "writableRoots" => ["~/.karkhana-workspaces"],
             "readOnlyAccess" => %{"type" => "fullAccess"},
             "networkAccess" => false,
             "excludeTmpdirEnvVar" => false,
             "excludeSlashTmp" => false
  ...
  test "runtime sandbox policy resolution passes explicit policies through unchanged" do
    test_root =
      Path.join(
        System.tmp_dir!(),
        "karkhana-runtime-sandbox-#{System.unique_integer([:positive])}"
      )
  ...
  test "path safety returns errors for invalid path segments" do
    ...
    assert {:error, {:path_canonicalize_failed, ^expanded_path, :enametoolong}} =
             Karkhana.PathSafety.canonicalize(path)
  end
  ...
  test "runtime sandbox policy resolution defaults when omitted and ignores workspace for explicit policies" do
    test_root =
      Path.join(
        System.tmp_dir!(),
        "karkhana-runtime-sandbox-branches-#{System.unique_integer([:positive])}"
      )
  ...
  test "remote workspace lifecycle uses ssh host aliases from worker config" do
    test_root =
      Path.join(
        System.tmp_dir!(),
        "karkhana-remote-workspace-#{System.unique_integer([:positive])}"
      )

    previous_path = System.get_env("PATH")
    previous_trace = System.get_env("KARKHANA_TEST_SSH_TRACE")

    on_exit(fn ->
      restore_env("PATH", previous_path)
      restore_env("KARKHANA_TEST_SSH_TRACE", previous_trace)
    end)

    try do
      trace_file = Path.join(test_root, "ssh.trace")
      fake_ssh = Path.join(test_root, "ssh")
      workspace_root = "~/.karkhana-remote-workspaces"
      workspace_path = "/remote/home/.karkhana-remote-workspaces/MT-SSH-WS"

      File.mkdir_p!(test_root)
      System.put_env("KARKHANA_TEST_SSH_TRACE", trace_file)
      System.put_env("PATH", test_root <> ":" <> (previous_path || ""))

      File.write!(fake_ssh, """
      #!/bin/sh
      trace_file="${KARKHANA_TEST_SSH_TRACE:-/tmp/karkhana-fake-ssh.trace}"
      printf 'ARGV:%s\\n' "$*" >> "$trace_file"

      case "$*" in
        *"__KARKHANA_WORKSPACE__"*)
          printf '%s\\t%s\\t%s\\n' '__KARKHANA_WORKSPACE__' '1' '#{workspace_path}'
          ;;
      esac
      ...
      assert trace =~ "__KARKHANA_WORKSPACE__"
      assert trace =~ "~/.karkhana-remote-workspaces/MT-SSH-WS"
